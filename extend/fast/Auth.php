...
class Auth
{
     ...
     *              array('uid'=>'用户id','group_id'=>'用户组id','extend_id'=>'扩展组id','name'=>'用户组名称','rules'=>'用户组拥有的规则id,多个,号隔开'),
     *              ...)
     */
    public function getGroups($uid)
    {
        ...
        $user_groups = Db::name($this->config['auth_group_access'])
                ...
                ->field('aga.uid,aga.group_id,aga.extend_id,ag.id,ag.pid,ag.name,ag.rules')
     ...
     * @return array
     */
    public function getVillageIds($uid = null)
    {
        $groups = $this->getGroups($uid);
        $villageIds = $ids = [];
        foreach ($groups as $K => $v)
        {
            switch ($v['id']) {
                case 6:
                    $ids = Db::name('village')->where('partner_id', $v['extend_id'])->column('id');
                    break;
                case 7:
                    $ids = Db::name('village')->where('company_id', $v['extend_id'])->column('id');
                    break;
                case 8:
                    $ids[] = $v['extend_id'];
                    break;
            ...
            trace(print_r($ids, true), 'log');
            if($ids) {
                $villageIds = $villageIds ? array_merge($villageIds, $ids) : $ids;
            }
        }
        return $villageIds;
    }
