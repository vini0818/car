...
class Ajax extends Backend
{
    ...
    public function wipecache()
    {
        ...
        $wipe_cache_path = array_merge($wipe_cache_type, config('api_cache_path'));
        foreach ($wipe_cache_path as $key => $item)
        {
            $dir = is_numeric($key) ? constant($item) : $item;
            if (!is_dir($dir))
                continue;
            rmdirs($dir);
        }
    ...
    public function dict()
    {
        ...
        $result = \app\common\model\Dict::getMagicConfig($name, $filter ? $filter : 'all');
        $dictlist = [];
        foreach ($result as $key => $value)
        {
            $dictlist[] = ['id' => $value['id'], 'name' => __($value['name'])];
        }
        $data = ['searchlist' => $dictlist];
