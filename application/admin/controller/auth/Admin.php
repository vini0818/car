    ...
    public function add()
    {
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                {
                    $dataset[] = ['uid' => $this->model->id, 'group_id' => $value, 'extend_id' => $extend[$value]];
                }
    ...
    public function edit($ids = NULL)
    {
        ...
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                foreach ($group as $value)
                {
                    $dataset[] = ['uid' => $row->id, 'group_id' => $value, 'extend_id' => $extend[$value]];
                }
                model('AuthGroupAccess')->saveAll($dataset);
                $this->success();
            }
        ...
        {
            $groupids[] = $v['id'];
            $extendids[$v['id']] = $v['extend_id'];
        }
        $this->view->assign("row", $row);
        $this->view->assign("groupids", $groupids);
        $this->view->assign("extendids", $extendids);
        return $this->view->fetch();
    }
