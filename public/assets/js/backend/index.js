define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
            ...
            //预警消息推送
            var socket = io(Config.msgserverurl);
            socket.on('connect', function () {
                var ret = socket.emit('login', Config.uid); //socket连接后以uid登录
                console.log(ret.connected ? 'login sucess' : 'login error');
            });
            socket.on('new_msg', function (msg) {
                console.log(msg);
                var span = $(".notifications-menu > a span");
                var num = span.text() ? parseInt(span.text()) : 0;
                span.text(num + 1);
                var item = '<li><a href="###" title="' + msg.title + '"><i class="' + msg.icon + '"></i>' + msg.title + '</a></li>';
                $(item).appendTo($(".notifications-menu ul.menu"));
                $(".notifications-menu ul.menu li a").unbind('click').bind('click', function () {
                    Layer.open({
                        title: '系统预警提醒',
                        area: ["500px", "auto"],
                        content: '<h5 style="background-color:#f7f7f7; font-size:14px; padding: 10px;">' + $(this).attr('title') + '请尽快处理！</h5>',
                        btn: ['好的，知道了', '忽略'],
